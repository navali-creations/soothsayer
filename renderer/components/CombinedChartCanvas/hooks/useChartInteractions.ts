...
import type { ChartLayout } from "../canvas-chart-utils";
import {
  BRUSH_TRAVELLER_WIDTH,
  clamp,
  MIN_ZOOM_WINDOW,
} from "../canvas-chart-utils";
import type { BrushRange, ChartDataPoint } from "../chart-types";
...
export function useChartInteractions({
  ...
  const hitTestChart = useCallback(
    (cx: number, cy: number): number | null => {
      ...
      if (bestDist < Math.max(20, layout.chartWidth / numVisible)) {
        return bestIdx;
      }
      return null;
    },
    [layout, visibleData.length, mapX, isInChartArea],
  );

  const brushXToIndex = useCallback(
    (cx: number): number => {
      const total = chartDataRef.current.length;
      if (total <= 1) return 0;
      const frac =
        (cx - layout.brushLeft) / (layout.brushRight - layout.brushLeft);
