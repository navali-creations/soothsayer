...
import { useCurrentSession, useSessionDetails } from "~/renderer/store";
...
export const TooltipInlineCard = memo(({ cardName }: { cardName: string }) => {
  const { getSession: getCurrentSession } = useCurrentSession();
  const { getSession: getDetailsSession } = useSessionDetails();
