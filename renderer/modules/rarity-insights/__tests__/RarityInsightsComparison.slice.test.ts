...
describe("RarityInsightsComparison.slice", () => {
  ...
  describe("getDifferences", () => {
    ...
    it("treats missing card in filter as rarity 4 (common)", async () => {
      ...
      const diffs = s().getDifferences();
      // House of Mirrors has poe.ninja rarity 2, filter defaults to 4 → different
      expect(diffs.has("House of Mirrors")).toBe(true);
      // Rain of Chaos has poe.ninja rarity 4, filter defaults to 4 → same
      expect(diffs.has("Rain of Chaos")).toBe(false);
    });
  });
  ...
  // ─── 19. getDisplayRowCount ──────────────────────────────────────────

  describe("getDisplayRowCount", () => {
    it("returns total card count when showDiffsOnly is false", () => {
      store = makeStoreWithCards();
      const count = store
        .getState()
        .rarityInsightsComparison.getDisplayRowCount();
      // 3 cards, none from boss, showDiffsOnly is false
      expect(count).toBe(3);
    });

    it("excludes boss cards when includeBossCards is false", () => {
      store = createTestStore({
        cards: {
          allCards: [
    ...
    it("filters by differences when showDiffsOnly is true", async () => {
      ...
      electron.rarityInsights.parse.mockResolvedValue({
        ...
        rarities: [
          { filterId: "filter-1", cardName: "The Doctor", rarity: 1 },
          { filterId: "filter-1", cardName: "House of Mirrors", rarity: 4 }, // different
      ...
      s().setShowDiffsOnly(true);
      const count = s().getDisplayRowCount();
      expect(count).toBe(1); // only "House of Mirrors" differs
    });
  });
  ...
  describe("getDisplayRows", () => {
    it("builds ComparisonRow[] combining card data with parsed filter rarities", async () => {
      store = makeStoreWithCards();
      electron = window.electron as unknown as ElectronMock;
      const s = () => store.getState().rarityInsightsComparison;
      ...
      const doctor = rows.find((r) => r.name === "The Doctor");
      expect(doctor).toBeDefined();
      expect(doctor!.rarity).toBe(1);
      expect(doctor!.filterRarities["filter-1"]).toBe(1);
      expect(doctor!.isDifferent).toBe(false);

      const hom = rows.find((r) => r.name === "House of Mirrors");
      expect(hom).toBeDefined();
      expect(hom!.rarity).toBe(2);
      expect(hom!.filterRarities["filter-1"]).toBe(4);
      expect(hom!.isDifferent).toBe(true);
    });

    it("returns null for filterRarities when filter is not yet parsed", () => {
      store = makeStoreWithCards();
      electron = window.electron as unknown as ElectronMock;
      const s = () => store.getState().rarityInsightsComparison;

      // Never resolve the parse
      electron.rarityInsights.parse.mockReturnValue(new Promise(() => {}));
      ...
      const rows = s().getDisplayRows();
      const doctor = rows.find((r) => r.name === "The Doctor");
      expect(doctor!.filterRarities["filter-1"]).toBeNull();
    });

    it("defaults missing card in parsed filter to rarity 4", async () => {
      store = makeStoreWithCards();
      electron = window.electron as unknown as ElectronMock;
      const s = () => store.getState().rarityInsightsComparison;

      // Only return Doctor, not House of Mirrors or Rain of Chaos
      electron.rarityInsights.parse.mockResolvedValue({
        filterId: "filter-1",
        filterName: "My Filter",
        totalCards: 1,
      ...
      const rows = s().getDisplayRows();
      const hom = rows.find((r) => r.name === "House of Mirrors");
      // Missing cards default to 4 (common)
      expect(hom!.filterRarities["filter-1"]).toBe(4);
    });

    it("filters by showDiffsOnly", async () => {
      store = makeStoreWithCards();
      electron = window.electron as unknown as ElectronMock;
      const s = () => store.getState().rarityInsightsComparison;
      ...
      electron.rarityInsights.parse.mockResolvedValue({
        ...
        rarities: [
          { filterId: "filter-1", cardName: "The Doctor", rarity: 1 },
          { filterId: "filter-1", cardName: "House of Mirrors", rarity: 4 }, // diff
          { filterId: "filter-1", cardName: "Rain of Chaos", rarity: 4 },
        ],
        hasDivinationSection: true,
      });
      ...
      expect(rows[0].name).toBe("House of Mirrors");
    });

    it("excludes boss cards by default", async () => {
      store = createTestStore({
        cards: {
          allCards: [
            makeCard({
    ...
    it("returns correct row shape with all expected fields", async () => {
      store = makeStoreWithCards();
      electron = window.electron as unknown as ElectronMock;
      const s = () => store.getState().rarityInsightsComparison;
      ...
      electron.rarityInsights.parse.mockResolvedValue({
        ...
        filterName: "My Filter",
        totalCards: 1,
        rarities: [{ filterId: "filter-1", cardName: "The Doctor", rarity: 2 }],
        hasDivinationSection: true,
      });
      ...
      expect(doctor).toEqual(
        expect.objectContaining({
          ...
          flavourHtml: "",
          rarity: 1,
          isDifferent: true, // poe.ninja rarity is 1, filter rarity is 2
          fromBoss: false,
        }),
      );
      expect(doctor.filterRarities).toEqual({ "filter-1": 2 });
      expect(doctor).toHaveProperty("prohibitedLibraryRarity");
    });
  });
});
