...
import type {
  ComparisonRow,
  ParsedRarityInsightsRarities,
} from "../../RarityInsightsComparison.slice/RarityInsightsComparison.slice";
...
const ComparisonTable = ({ globalFilter }: ComparisonTableProps) => {
  ...
  const priorityRarity = useBoundStore(
    (s) => s.rarityInsightsComparison.priorityPoeNinjaRarity,
  );
  const priorityPLRarity = useBoundStore(
    (s) => s.rarityInsightsComparison.priorityPlRarity,
  );
  const sorting = useBoundStore((s) => s.rarityInsightsComparison.tableSorting);
  const handleRarityClick = useBoundStore(
    (s) => s.rarityInsightsComparison.handlePoeNinjaRarityClick,
  );
  const handlePLRarityClick = useBoundStore(
    (s) => s.rarityInsightsComparison.handlePlRarityClick,
  );
  const handleFilterRarityClick = useBoundStore(
    (s) => s.rarityInsightsComparison.handleFilterRarityClick,
  );
  const priorityFilterRarities = useBoundStore(
    (s) => s.rarityInsightsComparison.priorityFilterRarities,
  );
  const handleSortingChange = useBoundStore(
    (s) => s.rarityInsightsComparison.handleTableSortingChange,
  );
  ...
  const storeSelectedFilters = useBoundStore(
    (s) => s.rarityInsightsComparison.selectedFilters,
  );
  const storeParsingFilterId = useBoundStore(
    (s) => s.rarityInsightsComparison.parsingFilterId,
  );
  ...
  const updateFilterCardRarity = useBoundStore(
    (s) => s.rarityInsightsComparison.updateFilterCardRarity,
  );
  ...
  const columns = useMemo(() => {
    ...
    return [
      ...
      columnHelper.accessor("name", {
        id: "name",
        header: "Card Name",
        cell: (info) => <RarityInsightsCardNameCell card={info.row.original} />,
        size: 200,
        minSize: 150,
