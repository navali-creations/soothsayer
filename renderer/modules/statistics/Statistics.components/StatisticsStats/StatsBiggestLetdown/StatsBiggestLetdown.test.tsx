...
import type { BiggestLetdownHighlight } from "../../../Statistics.types";
import { StatsBiggestLetdown } from "./StatsBiggestLetdown";
...
function createData(
  overrides: Partial<BiggestLetdownHighlight> = {},
): BiggestLetdownHighlight {
  return {
    sessionId: "session-wg-1",
...
describe("StatsBiggestLetdown", () => {
  ...
  it('renders "Biggest Letdown" title', () => {
    renderWithProviders(<StatsBiggestLetdown data={null} />);
  ...
  it("shows formatted profit in divines when chaosPerDivine > 0", () => {
    ...
    renderWithProviders(<StatsBiggestLetdown data={data} />);
  ...
  it("shows deck count in description when data is provided", () => {
    const data = createData({ totalDecksOpened: 450 });
    renderWithProviders(<StatsBiggestLetdown data={data} />);

    expect(screen.getByText("450 decks opened")).toBeInTheDocument();
  });
  ...
  it("navigates to session detail when clicked and data is provided", async () => {
    const data = createData();
    const { user } = renderWithProviders(<StatsBiggestLetdown data={data} />);

    await user.click(screen.getByTestId("stat"));

    expect(mockNavigate).toHaveBeenCalledWith({
      to: "/sessions/$sessionId",
      params: { sessionId: "session-wg-1" },
    });
  });

  it("does not navigate when clicked and data is null", async () => {
    const { user } = renderWithProviders(<StatsBiggestLetdown data={null} />);
