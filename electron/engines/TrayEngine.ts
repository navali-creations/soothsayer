...
import { AppEngine, MainWindowEngine, type MainWindowEngineType } from ".";

class TrayEngine {
  private tray: Tray | null = null;
  private readonly mainWindow: MainWindowEngineType;
  private readonly app: AppEngine;
  // private trayIconPath: string = path.join(__dirname, "../../icon512x512.png");
  // private macosTrayIconPath: string = path.join(
  // __dirname,
  // "../../tray_mac.png",
  // );
  private static _instance: TrayEngine;

  static getInstance() {
    if (!TrayEngine._instance) {
      TrayEngine._instance = new TrayEngine();
    }

    return TrayEngine._instance;
  }
  constructor() {
    this.mainWindow = MainWindowEngine.getInstance();
    this.app = AppEngine.getInstance();
  }
