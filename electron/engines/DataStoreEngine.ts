...
class DataStoreEngine {
  private static _instance: DataStoreEngine;
  ...
  static getInstance() {
    if (!DataStoreEngine._instance) {
      DataStoreEngine._instance = new DataStoreEngine();
    }

    return DataStoreEngine._instance;
  }
  ...
  private setupHandlers() {
    ...
    // Get all-time stats
    ipcMain.handle("data-store:get-all-time", (_event, game: GameType) => {
      return this.getAllTimeStats(game);
    });

    // Get league stats
    ipcMain.handle(
      "data-store:get-league",
