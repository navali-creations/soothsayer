...
# Use a local-only filename that Vite does not load for `pnpm start`.
LOCAL_ENV_FILE=".env.supabase.local"

if [ -f "$LOCAL_ENV_FILE" ]; then
    echo -e "${YELLOW}[!]${NC} $LOCAL_ENV_FILE already exists - skipping"
elif [ -f ".env.local" ]; then
    echo "[*] Migrating .env.local to $LOCAL_ENV_FILE..."
    mv ".env.local" "$LOCAL_ENV_FILE"
    echo -e "${GREEN}[OK]${NC} Local environment migrated"
else
    echo "[*] Creating $LOCAL_ENV_FILE..."
    ...
    if [ -z "$PUBLISHABLE_KEY" ]; then
        ...
        echo "Please run 'pnpm supabase status' and copy the key manually to $LOCAL_ENV_FILE"
    else
...
EOF
        echo -e "${GREEN}[OK]${NC} $LOCAL_ENV_FILE created"
    fi
fi
...
echo "     - soothsayer.local.db  → local Supabase (localhost)"
echo "     - soothsayer.db        → dev with production credentials"
echo "     - soothsayer.prod.db   → installed release build"
