DROP FUNCTION IF EXISTS merge_community_card_data(UUID, JSONB);
DROP FUNCTION IF EXISTS merge_community_upload_data(
  UUID,
  TEXT,
  TEXT,
  TEXT,
...
DROP FUNCTION IF EXISTS community_upload_limits();

CREATE OR REPLACE FUNCTION community_upload_limits()
RETURNS TABLE (
  max_cards INTEGER,
  max_card_count INTEGER,
  max_total_cards INTEGER
)
LANGUAGE sql
STABLE
SECURITY DEFINER
...
REVOKE EXECUTE ON FUNCTION community_upload_limits()
FROM PUBLIC, anon, authenticated;

GRANT EXECUTE ON FUNCTION community_upload_limits()
TO service_role;

COMMENT ON FUNCTION community_upload_limits()
IS 'Validation limits shared by upload-community-data and merge_community_upload_data. Keep these values in sync with the edge function constants.';
...
BEGIN
  ...
  WITH deduped AS (
    SELECT
      card.card_id,
      MAX(card.count)::INTEGER AS count
    FROM jsonb_to_recordset(p_cards) AS card(
      card_id UUID,
      count INTEGER
    )
...
REVOKE EXECUTE ON FUNCTION merge_community_upload_data(
  UUID,
  TEXT,
  TEXT,
  TEXT,
...
GRANT EXECUTE ON FUNCTION merge_community_upload_data(
  UUID,
  TEXT,
  TEXT,
  TEXT,
...
COMMENT ON FUNCTION merge_community_upload_data(
  ...
  BOOLEAN,
  JSONB
) IS 'Atomically creates or updates community_uploads, merges community_card_data with GREATEST semantics, and returns the server-side upload summary. SECURITY DEFINER, service_role only.';
