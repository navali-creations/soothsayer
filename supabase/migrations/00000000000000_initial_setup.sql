...
BEGIN
  ...
  VALUES (v_user_id, p_endpoint, p_app_version, p_ip)
  RETURNING id INTO v_request_id;
  RETURN json_build_object('success', true, 'request_id', v_request_id, 'message', 'API request logged successfully');
END;
$$;

GRANT EXECUTE ON FUNCTION log_api_request(TEXT, TEXT, TEXT) TO authenticated, service_role;

COMMENT ON FUNCTION log_api_request(TEXT, TEXT, TEXT) IS 'SECURITY DEFINER with auth.uid() validation. User cannot impersonate others.';
...
-- Cron function: create snapshots for all active leagues
...
-- Cron function: sync leagues from API
