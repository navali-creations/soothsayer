...
import type { RarityInsightsFilterType } from "./RarityInsights.dto";
...
/**
 * RarityInsightsScanner handles file system operations for discovering and reading
 ...
 */
export class RarityInsightsScanner {
  ...
  async scanLocalFilters(game: GameType): Promise<ScannedFilterMetadata[]> {
    ...
    if (!(await this.directoryExists(dir))) {
      console.log(
        `[RarityInsightsScanner] Local filters directory does not exist: ${dir}`,
      );
      return [];
    }
    ...
    try {
      ...
      console.error(
        `[RarityInsightsScanner] Failed to scan local filters in ${dir}:`,
        error,
      );
      return [];
    }
  ...
  async scanOnlineFilters(game: GameType): Promise<ScannedFilterMetadata[]> {
    ...
    if (!(await this.directoryExists(dir))) {
      console.log(
        `[RarityInsightsScanner] Online filters directory does not exist: ${dir}`,
      );
      return [];
    }
    ...
    try {
      ...
      console.error(
        `[RarityInsightsScanner] Failed to scan online filters in ${dir}:`,
        error,
      );
      return [];
    }
  ...
  async extractLocalMetadata(
    ...
    try {
      ...
      console.warn(
        `[RarityInsightsScanner] Failed to read local filter metadata: ${filePath}`,
        error,
      );
      return null;
    }
  ...
  async extractOnlineMetadata(
    ...
    try {
      ...
      const parsed = RarityInsightsScanner.parseOnlineHeader(headerLines);
      ...
      console.warn(
        `[RarityInsightsScanner] Failed to read online filter metadata: ${filePath}`,
        error,
      );
      return null;
    }
