...
import { RarityInsightsParser, type TierBlock } from "../RarityInsights.parser";
...
describe("RarityInsightsParser", () => {
  ...
  describe("findDivinationSectionId", () => {
    it("should extract section ID from a valid TOC", () => {
      ...
      const result = RarityInsightsParser.findDivinationSectionId(lines);
    ...
    it("should return null for empty lines", () => {
      const result = RarityInsightsParser.findDivinationSectionId([]);
      expect(result).toBeNull();
    });
  ...
  describe("findSectionStart", () => {
    it("should find the section header by ID", () => {
      ...
      const result = RarityInsightsParser.findSectionStart(lines, "4200");
    ...
    it("should return -1 for empty lines", () => {
      const result = RarityInsightsParser.findSectionStart([], "4200");
      expect(result).toBe(-1);
    });
  ...
  describe("extractSectionLines", () => {
    it("should extract lines from section header to the next section", () => {
      ...
      const result = RarityInsightsParser.extractSectionLines(lines, 0);
    ...
    it("should not include the section header itself in the results", () => {
      ...
      const result = RarityInsightsParser.extractSectionLines(lines, 1);
  ...
  describe("parseTierBlocks", () => {
    ...
    it("should return empty array for empty input", () => {
      const result = RarityInsightsParser.parseTierBlocks([]);
      expect(result).toEqual([]);
    });
  ...
  describe("extractCardNames", () => {
    it("should extract a single card name", () => {
      const result = RarityInsightsParser.extractCardNames(
        'BaseType == "The Doctor"',
      );
    ...
    it("should return empty array for line with no quotes", () => {
      const result = RarityInsightsParser.extractCardNames("SetFontSize 45");
      expect(result).toEqual([]);
    });
    ...
    it("should not produce results for empty quoted strings", () => {
      ...
      const emptyResult = RarityInsightsParser.extractCardNames('""');
    ...
    it("should handle many card names on one line", () => {
      ...
      const result = RarityInsightsParser.extractCardNames(line);
    ...
    it("should be called multiple times without regex state leaking", () => {
      ...
      const result1 = RarityInsightsParser.extractCardNames(
        'BaseType == "Card A"',
      );
      const result2 = RarityInsightsParser.extractCardNames(
        'BaseType == "Card B"',
      );
      const result3 = RarityInsightsParser.extractCardNames(
        'BaseType == "Card C"',
      );
  ...
  describe("mapTierToRarity", () => {
    it("should map t1 to rarity 1 (Extremely Rare)", () => {
      expect(RarityInsightsParser.mapTierToRarity("t1")).toBe(1);
    });

    it("should map t2 to rarity 2 (Rare)", () => {
      expect(RarityInsightsParser.mapTierToRarity("t2")).toBe(2);
    });

    it("should map t3 to rarity 2 (Rare)", () => {
      expect(RarityInsightsParser.mapTierToRarity("t3")).toBe(2);
    });

    it("should map tnew to rarity 2 (Rare)", () => {
      expect(RarityInsightsParser.mapTierToRarity("tnew")).toBe(2);
    });

    it("should map t4c to rarity 3 (Less Common)", () => {
      expect(RarityInsightsParser.mapTierToRarity("t4c")).toBe(3);
    });

    it("should map t5c to rarity 4 (Common)", () => {
      expect(RarityInsightsParser.mapTierToRarity("t5c")).toBe(4);
    });

    it("should map t4 to rarity 4 (Common)", () => {
      expect(RarityInsightsParser.mapTierToRarity("t4")).toBe(4);
    });

    it("should map t5 to rarity 4 (Common)", () => {
      expect(RarityInsightsParser.mapTierToRarity("t5")).toBe(4);
    });

    it("should map restex to rarity 4 (Common)", () => {
      expect(RarityInsightsParser.mapTierToRarity("restex")).toBe(4);
    });

    it("should return null for exstack (ignored)", () => {
      expect(RarityInsightsParser.mapTierToRarity("exstack")).toBeNull();
    });

    it("should return null for excustomstack (ignored)", () => {
      expect(RarityInsightsParser.mapTierToRarity("excustomstack")).toBeNull();
    });

    it("should return null for unknown tier names", () => {
      expect(RarityInsightsParser.mapTierToRarity("t99")).toBeNull();
      expect(RarityInsightsParser.mapTierToRarity("unknown")).toBeNull();
      expect(RarityInsightsParser.mapTierToRarity("")).toBeNull();
    });
  ...
  describe("buildRarityMap", () => {
    it("should build a map from tier blocks", () => {
      ...
      const result = RarityInsightsParser.buildRarityMap(blocks);
    ...
    it("should return empty map for empty input", () => {
      const result = RarityInsightsParser.buildRarityMap([]);
  ...
  describe("getTierMapping", () => {
    it("should return a copy of the tier mapping", () => {
      const mapping = RarityInsightsParser.getTierMapping();
    ...
    it("should return a new object each time (not the same reference)", () => {
      const mapping1 = RarityInsightsParser.getTierMapping();
      const mapping2 = RarityInsightsParser.getTierMapping();
    ...
    it("should not allow modification of internal mapping", () => {
      ...
      const fresh = RarityInsightsParser.getTierMapping();
  ...
  describe("parseFilterFile", () => {
    ...
    it("should return empty result when file does not exist", async () => {
      const result = await RarityInsightsParser.parseFilterFile(
    ...
    it("should read and parse a valid filter file from disk", async () => {
      ...
      const result = await RarityInsightsParser.parseFilterFile(tmpFile);
