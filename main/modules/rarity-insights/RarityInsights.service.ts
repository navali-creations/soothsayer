...
import { RarityInsightsRepository } from "./RarityInsights.repository";
import { RarityInsightsScanner } from "./RarityInsights.scanner";
...
class RarityInsightsService {
  private static _instance: RarityInsightsService;
  private repository: RarityInsightsRepository;
  private scanner: RarityInsightsScanner;
  private settingsStore: SettingsStoreService;
  private divinationCards: DivinationCardsService;

  static getInstance(): RarityInsightsService {
    if (!RarityInsightsService._instance) {
      RarityInsightsService._instance = new RarityInsightsService();
    }
    return RarityInsightsService._instance;
  }

  constructor() {
    const db = DatabaseService.getInstance();
    this.repository = new RarityInsightsRepository(db.getKysely());
    this.scanner = new RarityInsightsScanner();
  ...
  private setupIpcHandlers(): void {
    ...
    ipcMain.handle(RarityInsightsChannel.ScanRarityInsights, async () => {
      return await this.scanFilters();
    });

    // Get all discovered filters from the database
    ipcMain.handle(RarityInsightsChannel.GetAllRarityInsights, async () => {
    ...
    ipcMain.handle(RarityInsightsChannel.GetRaritySource, async () => {
   ...
   */
  public async scanFilters(): Promise<RarityInsightsScanResultDTO> {
    console.log("[RarityInsightsService] Starting filter directory scan...");
    ...
    console.log(
      `[RarityInsightsService] Found ${scannedFilters.length} filter files on disk`,
    );

    // Phase 2: Upsert metadata into the database
    const upsertData = scannedFilters.map((scanned) => ({
      id: RarityInsightsScanner.generateFilterId(scanned.filePath),
    ...
    if (deletedCount > 0) {
      console.log(
        `[RarityInsightsService] Cleaned up ${deletedCount} stale filter entries`,
      );
    }
    ...
    const filters = allMetadata.map((metadata) =>
      RarityInsightsMapper.toDiscoveredRarityInsightsDTO(
        metadata,
        leagueStartDate,
      ),
    );
    ...
    console.log(
      `[RarityInsightsService] Scan complete: ${localCount} local, ${onlineCount} online filters`,
    );
   ...
   */
  public async getAllFilters(): Promise<DiscoveredRarityInsightsDTO[]> {
    const leagueStartDate = await this.getLeagueStartDate();
    const allMetadata = await this.repository.getAll();
  ...
  public async parseFilter(
    filterId: string,
  ): Promise<RarityInsightsParseResultDTO> {
    ...
    console.log(
      `[RarityInsightsService] Parsing filter "${metadata.filterName}" (${filterId})...`,
    );

    // Delegate to RarityInsightsParser for the actual file parsing
  ...
  public async applyFilterRarities(
    ...
    console.log(
      `[RarityInsightsService] Applying filter rarities for ${game}/${league} from filter ${filterId}...`,
    );
    ...
    if (!parseResult.hasDivinationSection) {
      console.warn(
        `[RarityInsightsService] Filter "${parseResult.filterName}" has no divination section — cannot apply filter rarities`,
      );
      return {
        success: false,
        totalCards: 0,
    ...
    console.log(
      `[RarityInsightsService] Successfully applied filter rarities from "${parseResult.filterName}" (${parseResult.totalCards} cards)`,
    );
  ...
  public async ensureFilterParsed(
    filterId: string,
  ): Promise<RarityInsightsParseResultDTO | null> {
    const metadata = await this.repository.getById(filterId);
  ...
  public async selectFilter(filterId: string | null): Promise<void> {
    if (filterId !== null) {
      ...
      console.log(
        `[RarityInsightsService] Selected filter: "${metadata.filterName}" (${filterId})`,
      );
    } else {
      console.log("[RarityInsightsService] Cleared filter selection");
    }
   ...
   */
  public async getSelectedFilter(): Promise<RarityInsightsMetadataDTO | null> {
  ...
  public async setRaritySource(source: RaritySource): Promise<void> {
    ...
    console.log(`[RarityInsightsService] Rarity source changed to: ${source}`);
  ...
  private async getLeagueStartDate(): Promise<string | null> {
    try {
      ...
      console.warn(
        "[RarityInsightsService] Failed to get league start date for outdated detection:",
        error,
      );
      return null;
    }
  ...
  private emitFilterRaritiesApplied(data: {
    ...
    for (const window of windows) {
      try {
        ...
        console.warn(
          "[RarityInsightsService] Failed to emit filter rarities applied event:",
   ...
   */
  public getRepository(): RarityInsightsRepository {
