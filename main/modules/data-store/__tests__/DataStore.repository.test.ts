...
import { seedCards } from "~/main/modules/__test-utils__/create-test-db";
...
describe("DataStoreRepository", () => {
  const { getRepository, getTestDb } = setupRepositoryTest(DataStoreRepository);

  // ─── Global Stats Operations ─────────────────────────────────────────────

  describe("getGlobalStat", () => {
    it("should return the initialized totalStackedDecksOpened stat", async () => {
      const result = await getRepository().getGlobalStat(
    ...
    it("should return null for a non-existent stat key", async () => {
      const result = await getRepository().getGlobalStat("nonExistentKey");
  ...
  describe("incrementGlobalStat", () => {
    it("should increment the stat by the given amount", async () => {
      await getRepository().incrementGlobalStat("totalStackedDecksOpened", 5);
    ...
    it("should increment cumulatively across multiple calls", async () => {
      await getRepository().incrementGlobalStat("totalStackedDecksOpened", 3);
      await getRepository().incrementGlobalStat("totalStackedDecksOpened", 7);
      await getRepository().incrementGlobalStat("totalStackedDecksOpened", 1);
    ...
    it("should handle incrementing by large numbers", async () => {
      await getRepository().incrementGlobalStat(
        "totalStackedDecksOpened",
        100_000,
      );
  ...
  describe("resetGlobalStat", () => {
    it("should reset the stat value to 0", async () => {
      await getRepository().incrementGlobalStat("totalStackedDecksOpened", 42);
      await getRepository().resetGlobalStat("totalStackedDecksOpened");
    ...
    it("should not throw when resetting an already-zero stat", async () => {
      await expect(
        getRepository().resetGlobalStat("totalStackedDecksOpened"),
    ...
    it("should only reset the specified key", async () => {
      ...
      await getTestDb()
        .kysely.insertInto("global_stats")
        .values({ key: "otherStat", value: 99 })
        .execute();

      await getRepository().incrementGlobalStat("totalStackedDecksOpened", 50);
      await getRepository().resetGlobalStat("totalStackedDecksOpened");

      const resetStat = await getRepository().getGlobalStat(
        "totalStackedDecksOpened",
      );
      expect(resetStat!.value).toBe(0);

      const otherStat = await getRepository().getGlobalStat("otherStat");
  ...
  describe("upsertCard", () => {
    it("should insert a new card with count 1", async () => {
      await getRepository().upsertCard({
        game: "poe1",
        scope: "all-time",
        cardName: "The Doctor",
      ...
      const cards = await getRepository().getCardsByScope("poe1", "all-time");
    ...
    it("should increment count correctly across many upserts", async () => {
      ...
      const cards = await getRepository().getCardsByScope("poe1", "Settlers");
      expect(cards).toHaveLength(1);
    ...
    it("should keep same card separate across different scopes", async () => {
      ...
      const allTime = await getRepository().getCardsByScope("poe1", "all-time");
      const league = await getRepository().getCardsByScope("poe1", "Settlers");
    ...
    it("should keep same card separate across different games", async () => {
      ...
      const poe1Cards = await getRepository().getCardsByScope(
        "poe1",
        "all-time",
      );
      const poe2Cards = await getRepository().getCardsByScope(
        "poe2",
        "all-time",
      );
  ...
  describe("getCardsByScope", () => {
    ...
    it("should return only cards matching the game and scope", async () => {
      await seedCards(getTestDb().kysely, [
      ...
      const result = await getRepository().getCardsByScope("poe1", "all-time");
    ...
    it("should return all cards for a given scope", async () => {
      ...
      const result = await getRepository().getCardsByScope("poe1", "Settlers");
  ...
  describe("getTotalCountByScope", () => {
    it("should return 0 when no cards exist", async () => {
      const total = await getRepository().getTotalCountByScope(
        "poe1",
        "all-time",
      );
      expect(total).toBe(0);
  ...
  describe("getLastUpdatedByScope", () => {
    it("should return null when no cards exist", async () => {
      const result = await getRepository().getLastUpdatedByScope(
        "poe1",
        "all-time",
      );
      expect(result).toBeNull();
  ...
  describe("deleteCardsByScope", () => {
    it("should delete all cards for the specified game and scope", async () => {
      ...
      await getRepository().deleteCardsByScope("poe1", "all-time");
    ...
    it("should not delete cards from other scopes", async () => {
      ...
      const remainingCards = await getRepository().getCardsByScope(
        "poe1",
        "Settlers",
      );
    ...
    it("should not throw when deleting from an empty scope", async () => {
      await expect(
        getRepository().deleteCardsByScope("poe1", "nonexistent"),
      ).resolves.not.toThrow();
    });
  });
  ...
  describe("getAvailableLeagues", () => {
    it("should return empty array when no cards exist", async () => {
      const leagues = await getRepository().getAvailableLeagues("poe1");
    ...
    it("should not return leagues from other games", async () => {
      ...
      const poe1Leagues = await getRepository().getAvailableLeagues("poe1");
      const poe2Leagues = await getRepository().getAvailableLeagues("poe2");
  ...
  describe("cascading upserts (simulating DataStoreService.addCard flow)", () => {
    it("should support the full cascade: global → all-time → league", async () => {
      ...
      // Verify global stats
      const globalStat = await getRepository().getGlobalStat(
        "totalStackedDecksOpened",
      );
      expect(globalStat!.value).toBe(1);

      // Verify all-time
      const allTimeCards = await getRepository().getCardsByScope(
        game,
        "all-time",
      );
      ...
      // Verify league
      const leagueCards = await getRepository().getCardsByScope(game, league);
    ...
    it("should maintain correct counts after multiple card additions", async () => {
      ...
      const allTimeTotal = await getRepository().getTotalCountByScope(
        game,
        "all-time",
      );
      ...
      const leagueTotal = await getRepository().getTotalCountByScope(
        game,
        league,
      );
    ...
    it("should isolate card counts across multiple leagues", async () => {
      ...
      expect(await getRepository().getTotalCountByScope(game, "Settlers")).toBe(
        2,
      );
      expect(
        await getRepository().getTotalCountByScope(game, "Necropolis"),
      ).toBe(1);
      expect(await getRepository().getTotalCountByScope(game, "all-time")).toBe(
        3,
      );
    });
  });
  ...
  describe("transactional operations", () => {
    it("should support using repository within a Kysely transaction", async () => {
      ...
      const leagueCards = await getRepository().getCardsByScope(
        "poe1",
        "Settlers",
      );
      expect(leagueCards).toHaveLength(1);
